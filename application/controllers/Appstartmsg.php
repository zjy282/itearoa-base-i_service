...
class AppstartMsgController extends \BaseController {
	...
	public function getEffectiveAppStartMsgAction() {
		...
		// 获取物业可用信息
		$hotelMsg = $this->model->getAppstartMsgList ( array ('status' => 1,'type' => 1,'limit' => 5,'dataid' => $hotelId ) );
		// 获取集团可用信息
		$groupMsg = $this->model->getAppstartMsgList ( array ('status' => 1,'type' => 2,'limit' => 5,'dataid' => $hotelId ) );
		...
		if ($data ['list']) {
			$appStartMsgLogModel->addAppstartMsgLog ( array ('msgid' => $data ['list'] [0] ['msgId'],'platform' => $platform,'identity' => $identity ) );
		}
		$this->echoSuccessData ( $data );
	}
}
