 ...
 * 集团新闻标签管理数据层
 */
class Dao_GroupNewsTag extends Dao_Base {
    ...
    public function getNewsTagList(array $param): array {
        ...
        $sql = "select * from group_news_tag {$whereList['sql']}";
    ...
    public function getNewsTagCount(array $param): int {
    	$whereList = $this->getListWhereSql($param);
    	$sql = "select count(1) as count from group_news_tag {$whereList['sql']}";
    	$result = $this->db->fetchAssoc($sql, $whereList['case']);
    	return intval($result['count']);
    }

    /**
     * 根据id查询group_news_tag详情
     *
     * @param
     *            int id
     * @return array
    ...
    public function getNewsTagDetail(int $id): array {
        ...
        if ($id) {
            $sql = "select * from group_news_tag where id=?";
    ...
    public function updateNewsTagById(array $info, int $id) {
        ...
        if ($id) {
            $result = $this->db->update('group_news_tag', $info, array ( 'id' => $id) );
        }
    ...
    public function addNewsTag(array $info) {
        $this->db->insert('group_news_tag', $info);
