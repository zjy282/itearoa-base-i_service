...
/**
 * Class Dao_GroupActivity
 * 活动管理数据层
 */
class Dao_GroupActivity extends Dao_Base {
    ...
    public function getActivityCount(array $param): int {
        $paramSql = $this->handlerActivityListParams($param);
        $sql = "select count(1) as count from group_activity {$paramSql['sql']}";
    ...
    public function getActivityDetail(int $id): array {
        ...
        if ($id) {
            $sql = "select * from group_activity where id=?";
    ...
    public function updateActivityById(array $info, int $id) {
        ...
        if ($id) {
            $result = $this->db->update('group_activity', $info, array('id' => $id));
        }
    ...
    public function addActivity(array $info) {
        $this->db->insert('group_activity', $info);
