...
//加载conf.inc.php文件
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'conf'.DIRECTORY_SEPARATOR.'conf.inc.php';
...
class Oss_Ali{
    ...
    public function auth($options){
        ...
        //创建请求
        $request = new Oss_Lib_RequestCore($this->request_url);
        ...
        if (isset($options[self::OSS_FILE_UPLOAD])){
            if (is_resource($options[self::OSS_FILE_UPLOAD])){
                ...
                if (isset($headers[self::OSS_CONTENT_TYPE]) && ($headers[self::OSS_CONTENT_TYPE] === 'application/x-www-form-urlencoded')){
                    ...
                    $mime_type = Oss_MimeTypes::get_mimetype($extension);
    ...
    private function log($msg){
        ...
        if(defined('ALI_LOG_PATH') ){
            ...
            $log_path = $sysConfig->application->log->path;
